...
getTCGA <- function(disease="GBM", data.type="RNASeq2", type="", filter="Y", p=getOption("mc.cores", 2), clinical=FALSE, cvars="OS"){
  ...
  # Parse links
  ldoc <- tryCatch({
    ldoc <- xml2::read_html("https://gdac.broadinstitute.org/runs/stddata__latest/", useInternalNodes = TRUE)
  } ,error = function(e){
    ldoc = NULL
  })
  
  if(is.null(ldoc)){
    message("Error: Problem connecting to Firehose. Please ensure Internet connection is working.\n")
    dat <- NULL
    return(dat)
  }
  
  # parse to get data set and URL for each datasets
  datasets <- xml2::xml_find_all(x = ldoc, xpath = "//a[contains(@href, 'Standardized+Data+Run+Release+Notes')]")  %>% xml_text()
  ...
  dataset <- disease
  llinks = unlist(xml2::xml_find_all(ldoc, "//a[@href]")%>%  xml2::xml_attr(attr = "href")) 
  dlinks = llinks[grepl(paste("/data/", dataset, "/", sep=""), llinks)]
  dlinks = gsub("http:", "https:", dlinks)
  ddoc = xml2::read_html(dlinks, useInternalNodes = T)
...
RNASeqV2 <- function(ddoc, dlinks, dataset){
  ...
  plinks = xml2::xml_find_all(ddoc, keyWord) %>% xml_attr(attr = "href")
